  ...
  <?php foreach($cats as $id => $cat) { ?>
    <div class="span3 box-cat">
	  <div class="head">
		...
		<div class="child"> &nbsp;<?php if (isset($list_child[$id]) AND count($list_child[$id])) echo implode(' &middot; ', $list_child[$id]); ?><hr /></div>
		<?php if (count($cat['newest_post'])) {
			echo '<ul class="newest_post">';
			foreach($cat['newest_post']['cat_box'] as $post) {
				echo '<li>'. $post .'</li>';
			}
			echo '</ul>';
		} ?>
